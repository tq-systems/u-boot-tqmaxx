...
static inline
bool tq_vard_has_discrete_tempsensor(const struct tq_vard *vard)
{
	return (tq_vard_has_feature(vard, 4, 1) > 0);
}
...
struct __packed tq_eeprom_data {
	union {
		...
		static_assert(sizeof(struct tq_vard) == TQ_EE_HRCW_BYTES,
