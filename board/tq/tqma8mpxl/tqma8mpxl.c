...
static int fixup_ecc_reserved_mem(void *blob, struct bd_info *bis)
{
	int ret = 0;
	...
	if (IS_ENABLED(CONFIG_IMX8M_DRAM_INLINE_ECC)) {
		 ...
		 */
		carveout.start = CFG_SYS_SDRAM_BASE + ram_size;
		carveout.end = carveout.start + ram_size / 7ULL;

		ret = fdtdec_add_reserved_memory(blob, "ecc", &carveout,
						 NULL, 0, NULL,
