...
/*
 * Copyright (c) 2021-2024 TQ-Systems GmbH <u-boot@ew.tq-group.com>,
...
#if IS_ENABLED(CONFIG_TQMA8MPXL_RAM_1024MB)
	DRAM_INFO_ENTRY(1),
#endif
#if IS_ENABLED(CONFIG_TQMA8MPXL_RAM_2048MB)
	DRAM_INFO_ENTRY(2),
#endif
#if IS_ENABLED(CONFIG_TQMA8MPXL_RAM_4096MB)
	DRAM_INFO_ENTRY(4),
#endif
#if IS_ENABLED(CONFIG_TQMA8MPXL_RAM_8192MB)
	DRAM_INFO_ENTRY(8)
#endif
};
...
#if IS_ENABLED(CONFIG_TQMA8MPXL_RAM_MULTI)
...
static void spl_dram_init(int memtype)
{
	...
	} else if (IS_ENABLED(CONFIG_TQMA8MPXL_RAM_SINGLE_2GB)) {
	 ...
	 * in here.
	 */
	tqma8mpxl_dram_info[idx].table->ddrphy_trained_csr = ddr_ddrphy_trained_csr;
	tqma8mpxl_dram_info[idx].table->ddrphy_trained_csr_num = ddr_ddrphy_trained_csr_num;
