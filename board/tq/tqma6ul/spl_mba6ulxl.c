...
static const iomux_v3_cfg_t mba6ulxl_uart_pads[] = {
...
{
	imx_iomux_v3_setup_multiple_pads(mba6ulxl_uart_pads,
					 ARRAY_SIZE(mba6ulxl_uart_pads));
}

/* SD card on USDHC1 */
static const iomux_v3_cfg_t mba6ulxl_usdhc1_pads[] = {
...
static struct fsl_esdhc_cfg mba6ulxl_usdhc1_cfg = {
...
int tq_bb_board_mmc_init(struct bd_info *bis)
{
	...
	mba6ulxl_usdhc1_cfg.sdhc_clk = mxc_get_clock(MXC_ESDHC_CLK);
	if (fsl_esdhc_initialize(bis, &mba6ulxl_usdhc1_cfg))
		printf("Warning: failed to initialize SD card\n");
...
{
	/* Longest FDT name */
	char dt[] = "imx6ull-tqma6ull2l-mba6ulxl";
	enum tqma6ul_som_type somtype;

	somtype = tqma6ul_dt(dt, sizeof(dt), "mba6ulxl");
	if (somtype == tqma6_som_type_unknown)
		return -1;

	if (somtype != tqma6_som_type_lga) {
		printf("wrong TQMa6UL variant\n");
		return -1;
	}
