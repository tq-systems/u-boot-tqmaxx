...
static void tqma6ul_512_init_ddr_controller(void)
{
	/* TQMa6ul DDR config */
	...
	init_write_reg(0x021B083C, 0x41580150);
	init_write_reg(0x021B0848, 0x40404E52);
	init_write_reg(0x021B0850, 0x40404E4A);

#elif IS_ENABLED(CONFIG_MX6ULL)

	debug("SPL: tqma6ull ddr calibration standard variant ....\n");

	init_write_reg(0x021B080C, 0x00090009);
	init_write_reg(0x021B083C, 0x4140013C);
	init_write_reg(0x021B0848, 0x40403A3E);
	init_write_reg(0x021B0850, 0x40402E26);
#else
#error
#endif /* IS_ENABLED(CONFIG_MX6UL) */
	...
	debug("SPL: tqma6ull ddr calibration lga variant ....\n");

	init_write_reg(0x021B080C, 0x00050009);
	init_write_reg(0x021B083C, 0x41340130);
	init_write_reg(0x021B0848, 0x40403A3E);
	init_write_reg(0x021B0850, 0x40402E28);
...
#error
	...
	init_write_reg(0x021B0040, 0x0000004F);
	init_write_reg(0x021B0000, 0x84180000);
...
void tqma6ul_512_init(void)
{
	tqma6ul_512_init_ddr_controller();
}
