...
static int mblx2160a_lookup_gpio(const char *name, struct gpio_desc *desc)
{
	int ret = -ENOENT;
	int i;
...
static int mblx2160a_set_gpio(const char *name, int val)
{
	...
	ret = mblx2160a_lookup_gpio(name, &desc);
	if (ret)
		return ret;
...
static int xfi_config(int xfi_nr)
{
	...
	const struct i2c_reg_setting xfi_retimer_settings[] = {
		{0xff, 0x0c, 0xff}, /* Write both channels */
		{0x00, 0x04, 0xff}, /* Reset channel registers */
		{0x0a, 0x0c, 0xff}, /* Assert CDR */
		{0x2f, 0xc0, 0xff}, /* Select 10.3125Gbps */
		{0x31, 0x20, 0xff}, /* Adapt Mode 1 */
		{0x2d, 0x07, 0xff}, /* VOD = 1300mV, disable Driver Short Circuit protection */
		{0x15, 0x47, 0xff}, /* DEM = -2.8dB */
		{0x0a, 0x00, 0xff}, /* Release CDR */
	};
