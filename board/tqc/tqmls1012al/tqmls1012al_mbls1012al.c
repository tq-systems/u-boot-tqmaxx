...
void tqmls1012al_bb_misc_init_r(void)
{
	struct tq_eeprom_data eedat_board;
	struct gpio_desc pcie_clk_pd;
	/* must hold largest field of eeprom data */
	char safe_string[0x41];
	int ret = -1;

	ret = tq_read_eeprom_at(CONFIG_SYS_I2C_EEPROM_BUS,
	...
	if (ret) {
		...
		/* Board ID */
		tq_parse_eeprom_id(&eedat_board, safe_string,
				   ARRAY_SIZE(safe_string));
		if (strncmp(safe_string, "MBLS1012AL", 10) == 0)
			tq_env_set("boardtype", safe_string);

		/* Board Serial# */
		if (tq_parse_eeprom_serial(&eedat_board, safe_string,
					   ARRAY_SIZE(safe_string)) == 0)
			tq_env_set("boardserial#", safe_string);
		else
			tq_env_set("boardserial#", "???");

		/* Board MAC */
		tq_set_ethaddr(&eedat_board, "eth2addr", 0);
		tq_set_ethaddr(&eedat_board, "eth3addr", 1);
		tq_set_ethaddr(&eedat_board, "eth4addr", 2);
		tq_set_ethaddr(&eedat_board, "eth5addr", 3);

		tq_show_eeprom(&eedat_board, "MBLS1012AL");
	}
