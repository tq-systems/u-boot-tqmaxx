...
static int tq_parse_eeprom_serial_new(const struct tqc_eeprom_data *eeprom,
				      char *buf)
{
	unsigned int i;
...
static int tq_parse_eeprom_serial_old(const struct tqc_eeprom_data *eeprom,
				      char *buf)
{
	unsigned int i;
...
int tqc_parse_eeprom_serial(const struct tqc_eeprom_data *eeprom,
			    char *buf, size_t len)
{
	if (!buf || !eeprom)
		return -EINVAL;
